...
inline void draw_point(vec3 pos, const uint32 color = COLOR_WHITE) {
	draw_point(&pos[0], color);
}

void draw_line(const float from[3], const float to[3], const uint32 color = COLOR_WHITE);
inline void draw_line(vec3 from, vec3 to, const uint32 color = COLOR_WHITE) {
...
inline void draw_triangle(vec3 v0, vec3 v1, vec3 v2, const uint32 color = COLOR_WHITE) {
...
void draw_axis_aligned_box(const float min_box[3], const float max_box[3], const uint32 color = COLOR_WHITE);
