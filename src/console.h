...
struct Console {
    ...
    void Draw(const char* title, int width, int height, float dt) {
        ...
        // Speed to to hide/show
        float speed = console_height * dt * 8.f;
        ...
        for (int i = 0; i < items.Size; i++) {
            ...
            if (strstr(item, "[note]"))
                ...
                col = ImColor(0.8f, 0.2f, 0.2f, 1.0f);
        ...
        // if (copy_to_clipboard)
        //    ImGui::LogFinish();
        if (scroll_to_bottom) ImGui::SetScrollHereY();
        ...
        if (console_fully_shown) {
            ...
            if (ImGui::InputText("##Input", input_buffer, IM_ARRAYSIZE(input_buffer),
                ...
                while (input_end > input_buffer && input_end[-1] == ' ') {
                    input_end--;
                }
                *input_end = 0;
                if (input_buffer[0]) ExecCommand(input_buffer);
                strcpy(input_buffer, "");
            }
            ImGui::PopItemWidth();
    ...
    case ImGuiInputTextFlags_CallbackCharFilter: {
        AddLog("pressed key '%i'", (int)data->EventChar);
        break;
    }
                */
            ...
            case ImGuiInputTextFlags_CallbackHistory: {
                ...
                if (data->EventKey == ImGuiKey_UpArrow) {
                    ...
                    if (history_pos != -1)
                        if (++history_pos >= history.Size) history_pos = -1;
                }

                // A better implementation would preserve the data on the current input line along with cursor position.
                if (prev_history_pos != history_pos) {
                    data->CursorPos = data->SelectionStart = data->SelectionEnd = data->BufTextLen =
                        (int)snprintf(data->Buf, (size_t)data->BufSize, "%s", (history_pos >= 0) ? history[history_pos] : "");
                    data->BufDirty = true;
                }
            }
        }
