...
void camera_controller_trackball(Camera* camera, TrackballControllerState* state) {
    ASSERT(camera);
    ASSERT(state);
    ...
    if (state->input.rotate_button) {
        ...
        vec3 look_at = camera->position - camera->orientation * vec3(0, 0, state->distance);
        camera->orientation = glm::normalize(camera->orientation * q);
        camera->position = look_at + transform_vec(camera->orientation, vec3(0, 0, state->distance));
    } else if (state->input.pan_button) {
        vec2 delta = ndc_curr - ndc_prev;
        delta.y = -delta.y;
        vec3 move = vec3(-delta.x, delta.y, 0);
        move = transform_vec(camera->orientation, move) * math::pow(state->distance * state->params.pan_scale, state->params.pan_exponent);
