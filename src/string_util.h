...
struct StrBuf {
    static constexpr int64_t MaxSize = Size;
    static_assert(MaxSize > 1, "Size of StrBuf must be more than 1");
    char buf[MaxSize] = {};
    ...
    constexpr StrBuf(const char (&cstr)[N]) noexcept  {
        constexpr auto length = N < MaxSize ? N : MaxSize - 1;
        cexpr_helper::cexpr_copy(buf, cstr, length);
        buf[length] = '\0';
    }

    constexpr StrBuf(const char* cstr) noexcept {
        int64_t len = cexpr_helper::cexpr_strnlen(cstr, MaxSize - 1);
    ...
    constexpr StrBuf(const StrBuf& other) noexcept {
        cexpr_helper::cexpr_copy(buf, other.buf, MaxSize);
    ...
    constexpr StrBuf& operator=(const char* cstr) noexcept {
        const int64_t len = cexpr_helper::cexpr_strnlen(cstr, MaxSize - 1);
        cexpr_helper::cexpr_copy(buf, cstr, len);
        buf[len] = '\0';
        return *this;
    }
    ...
    constexpr int64_t capacity() const noexcept { return MaxSize; }
    constexpr int64_t length() const noexcept { return cexpr_helper::cexpr_strnlen(buf, MaxSize); }
...
StrBuf<N>& StrBuf<N>::operator+=(str_t txt) {
    const int64_t offset = (int64_t)cexpr_helper::cexpr_strnlen((const char*)buf, MaxSize);
    const int64_t length = MaxSize - offset;
    if (length > 0) {
