...
UTEST(viamd_void_field, grid_tiles_the_box_exactly) {
    ...
    md_grid_t grid = {};
    ASSERT_TRUE(void_field_grid(&grid, &cell, NULL, 0, 5.0f));
    ...
    const vec3_t xyz[2] = { {0.0f, 0.0f, 0.0f}, {10.0f, 20.0f, 30.0f} };
    ASSERT_TRUE(void_field_grid(&grid, NULL, xyz, 2, 1.0f));
    ...
    // Nothing to cover
    EXPECT_FALSE(void_field_grid(&grid, NULL, xyz, 0, 1.0f));
}
...
UTEST(viamd_void_field, field_matches_brute_force_and_does_not_depend_on_the_tile_split) {
    ...
    md_grid_t grid = {};
    ASSERT_TRUE(void_field_grid(&grid, &cell, NULL, 0, 1.3f));
