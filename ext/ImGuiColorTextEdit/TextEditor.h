...
public:
	...
	typedef std::unordered_map<int, std::vector<Marker>> Markers;
	...
	void ClearMarkers() { mMarkers.clear(); };
	void AddMarker(const Marker& marker) {
		mMarkers[marker.line].push_back(marker);
	}
	void AddMarkers(Marker* markers, int num_markers) {
		for (int i = 0; i < num_markers; ++i) {
			mMarkers[markers[i].line].push_back(markers[i]);
		}
	}
...
private:
	...
	struct EditorState
	{
		...
		Coordinates mMouseCursorPosition;
	};
