...
#define NOTIFY_TOAST_FLAGS				ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoSavedSettings
...
#define NOTIFY_NULL_OR_EMPTY(str)		(str == NULL || str[0] == '\0')
...
public:
	...
	NOTIFY_INLINE auto get_type() -> ImGuiToastType { return this->type; };
	...
	NOTIFY_INLINE auto get_elapsed_time() { return get_tick() - this->creation_time; }
...
public:
	...
	ImGuiToast(ImGuiToastType type, uint64_t dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		IM_ASSERT(type < ImGuiToastType_COUNT);

		this->type = type;
		this->dismiss_time = dismiss_time;
		this->creation_time = get_tick();
	...
	ImGuiToast(ImGuiToastType type, uint64_t dismiss_time, const char* format, ...) : ImGuiToast(type, dismiss_time) { NOTIFY_FORMAT(this->set_content, format); }
};
...
namespace ImGui
{
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (auto i = 0; i < static_cast<int>(notifications.size()); i++)
		{
