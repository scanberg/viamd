...
extern int read_xtc_natoms(const char *filename, int *num_atoms);

/* Seek through trajectory counting and indexing frames  */
extern int read_xtc_frame_offsets(const char *filename, int *num_frames, int64_t **offsets);
...
extern int write_xtc(XDRFILE *xd, int num_atoms, int step, float time, matrix box, rvec *x,
                     float prec);
