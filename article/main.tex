...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{VIAMD--VeloxChem Integration for Interactive Quantum Chemical Visualization and Analysis}
...
\begin{tocentry}
\centering
\IfFileExists{figure/toc.png}{%
\includegraphics[scale=0.135]{figure/toc.png}%
}{%
\fbox{\parbox[c][0.18\textheight][c]{0.72\textwidth}{\centering Placeholder for TOC graphic.}}%
}
\end{tocentry}
...
The visualization of results from electronic structure calculations is a cornerstone of quantum chemistry, helping researchers interpret molecular orbitals, electron densities, spectra, excited states, and related molecular properties. Traditional workflows often rely on standalone tools that interface with electronic structure programs to render orbital shapes and densities. Notable examples include Gaussian,~\cite{g16} which uses GaussView for orbital visualization, and programs such as ORCA,~\cite{ORCA} Q-Chem,~\cite{Q-Chem} and Turbomole,~\cite{Turbomole} which commonly generate grid-based files compatible with visualization tools such as VMD, PyMOL, Molden, Chimera, JSmol, Mol*, Avogadro, and dedicated wavefunction-analysis environments such as Multiwfn.~\cite{humphrey1996vmd,pymol,molden,Molden2000,pettersen2004ucsf,hanson2013jsmol,molstar,Hanwell2012Avogadro,Lu2012Multiwfn} Recent browser-first tools also show that on-the-fly orbital and density visualization is moving toward GPU-accelerated web runtimes, from WebGL-based molecular viewers to new WebGPU implementations, which improves accessibility but still leaves open questions on integrated analysis workflows across metadata, spectra, and transition descriptors.~\cite{Rego2015_3Dmoljs,MOrbVis2026} Broader surveys of molecular visualization underline the maturity of structure-oriented rendering and analysis workflows, while also highlighting the continued need for domain-specific interactive tools that connect data, representation, and interpretation.~\cite{kozlikova2017visualization} Spectral data are often handled separately through scripts or external plotting utilities, enabling the study of absorption, emission, circular dichroism, and other spectroscopic properties.
...
In this work, we present the integration of VIAMD with VeloxChem 1.0. VeloxChem is a Python-driven quantum chemistry program designed for high-performance simulations of molecular response properties at the Hartree--Fock, density-functional theory, and time-dependent density-functional theory levels.~\cite{Rinkevicius2020} It has been developed for efficient execution in high-performance computing environments, including recent GPU-accelerated implementations for demanding response calculations.~\cite{Velox-GPU} The integration described here provides native support in VIAMD for VeloxChem HDF5 output files, allowing molecular structures, orbitals, densities, spectra, and transition-analysis data to be loaded directly without conversion to external grid formats. Figure~\ref{fig:overview} provides an overview of this workflow in the VIAMD interface. The central spatial view shows a molecular electron-density isosurface directly associated with the loaded structure, while the surrounding panels expose complementary analysis tasks: the Response window for electronic spectroscopy, the Representations window for configuring orbital and density visualizations, the Transition Analysis window for attachment--detachment and fragment-based excited-state interpretation, and the Orbital Grid for side-by-side inspection of several orbitals. This overview captures the central idea of the integration: molecular structure, electronic representations, spectra, and state analysis remain connected within one interactive environment rather than being distributed across separate preprocessing and visualization tools.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figure/overview.png}
\caption{Overview of the VIAMD--VeloxChem interface for interactive quantum-chemical analysis. The central spatial view displays the molecular structure together with an electron-density isosurface, while the surrounding panels show the Response window for spectral analysis, the Representations window for configuring electronic visualizations, the Transition Analysis window for attachment--detachment and fragment-based excited-state analysis, and the Orbital Grid for comparative inspection of multiple orbitals.}
\label{fig:overview}
\end{figure}
...
\section{Loading Data and the Summary Window}
...
The Summary window, accessed through \texttt{Windows > VeloxChem > Summary}, provides the first overview of the loaded calculation. It reports the calculation level, including the method or functional and basis set, together with molecular metadata such as the number of atoms, charge, spin multiplicity, and number of electrons. The same window also exposes convergence information, including SCF convergence and, when present in the HDF5 file, geometry-convergence data for optimization-related calculations such as geometry optimizations, scan calculations, transition-state searches, and IRC paths. These views are summarized in Figure~\ref{fig:summary}.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.9\textwidth}
\centering
\includegraphics[width=\textwidth]{figure/summary.png}
\end{subfigure}
...
\centering
\includegraphics[width=\textwidth]{figure/scf-opt.png}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.495\textwidth}
\centering
\includegraphics[width=\textwidth]{figure/critical-point.png}
\end{subfigure}
\caption{Overview views available after loading a VeloxChem HDF5 dataset in VIAMD. Top: the Summary window reports calculation metadata, geometry information, and interactive tables linked to the molecular view. Bottom left: the SCF and optimization panel exposes convergence histories for electronic and structural relaxation. Bottom right: the Critical Points panel summarizes density-topology analysis derived from the reconstructed electron density, including graph settings, counts by critical-point type, and an interactive table of extracted points.}
\label{fig:summary}
\end{figure}

The Summary window also contains interactive tables linked to the three-dimensional molecular view. Selecting an entry in the geometry table highlights the corresponding atom in the spatial view, allowing numerical coordinates and molecular structure to be inspected together. In addition, VIAMD provides dedicated views for SCF and optimization convergence histories and computes critical point information from the electron density reconstructed from the molecular orbitals, following the long-established use of electron-density topology in quantum-chemical interpretation.\cite{Bader1990,Bader1991} The resulting critical-point table is likewise interactive with the 3D view. Together, these panels provide a compact entry point for checking calculation metadata, convergence behavior, and density-derived spatial features before moving to more specialized visualization and analysis tools.
...
\begin{subfigure}[t]{0.48\textwidth}
\centering
\IfFileExists{figure/figure-3b.png}{%
\includegraphics[width=\textwidth]{figure/figure-3b.png}%
}{%
...
\begin{subfigure}[t]{0.48\textwidth}
\centering
\IfFileExists{figure/figure-3c.png}{%
\includegraphics[width=\textwidth]{figure/figure-3c.png}%
}{%
...
\begin{subfigure}[t]{0.48\textwidth}
\centering
\IfFileExists{figure/figure-3d.png}{%
\includegraphics[width=\textwidth]{figure/figure-3d.png}%
}{%
...
\end{subfigure}
\caption{Representative electronic-structure visualizations in VIAMD. Top left: the default HOMO molecular-orbital representation generated upon loading a VeloxChem file. Top right: a green total-electron-density isosurface. Bottom left: an electron-density representation tinted by RESP charges. Bottom right: an alpha--beta density difference for an unrestricted calculation. Together, these examples show that the representation system can combine orbitals, densities, charge-derived coloring, and spin-density differences within the same interactive view.}
...
The same parsed data model drives the other VeloxChem windows. The Summary window queries calculation labels, electron counts, charge, multiplicity, SCF history, optimization energies, and optimization coordinates from \texttt{md\_vlx\_t}. Critical-point analysis in the Summary window follows a density-to-topology pipeline: VIAMD constructs a sampling grid from the molecular bounds, hashes grid and frame state to avoid unnecessary recomputation, evaluates the SCF AO density matrix on the grid with the same GTO machinery used for volumetric representations, and then passes the scalar field to the mdlib topology routines. The extracted structure is a Morse--Smale extremum graph with typed critical points (maxima, minima, split saddles, and join saddles) and connectivity edges; an optional simplification stage applies a user threshold and duplicate-saddle pruning before rendering and tabulation. On GPU-enabled builds, density evaluation and topology extraction are recorded and executed in one compute workflow before graph extraction to CPU-visible data. The Response window uses the response arrays for excitation energies, oscillator strengths, rotatory strengths, transition dipoles, CPP spectra, and vibrational IR data when those datasets are present. The Orbital Grid uses the same orbital coefficient accessors and GTO volume evaluation as the main representation system, but manages several small volumes simultaneously for side-by-side comparison. In this way, loading, summary inspection, orbital visualization, spectroscopy, and transition analysis all operate on one parsed VeloxChem HDF5 representation rather than on separate intermediate cube or plotting files.
...
Robin Skånberg was the main developer of the code and contributed to the code design. Gustav Eriksson implemented the electronic-structure graphical interface, including interactive spectra, vibrational analysis, and transition analysis. Mohit Sharma provided the first implementation and guidelines for the critical point analysis. Iulia Brumboiu, Josefine Hvarregaard Andersen, and Xin Li contributed to the VeloxChem interface and provided user feedback on the graphical interface. Talha Bin Masood advised on electronic-structure computation, visualization, and topological analysis. Patrick Norman initiated and co-led the project, and contributed to the VeloxChem interface. Mathieu Linares co-initiated and led the project and contributed to the code design.

\begin{acknowledgement}

%Please use ``The authors thank \ldots'' rather than ``The authors would like to thank \ldots''.
The authors gratefully acknowledge InfraVis, the Swedish national research infrastructure for scientific visualization, for supporting the development of VIAMD for interactive analysis workflows. VIAMD is developed at the KTH Center for Scientific Computing (KCSC) in close collaboration with the VeloxChem developers. The authors are also grateful to the broader computational chemistry community for the feedback, testing, encouragement, and discussions that helped shape the development of VIAMD.

\end{acknowledgement}

\section{Availability and Documentation}
The source code is freely available under the MIT license on the \href{https://github.com/scanberg/viamd}{VIAMD GitHub repository}.
As an open-source project, VIAMD warmly welcomes contributions, bug reports, feature requests, and documentation improvements.
...
The full dataset feature matrix has been moved to the Supporting Information, where it is provided as a standalone reference together with the mapping between VeloxChem documentation examples and the VIAMD workflows discussed in this article. Together, the documentation collection and the Supporting Information matrix define a reproducible route from VeloxChem calculations to VIAMD analysis: the article highlights representative examples for the main workflows, while the full set remains available as a practical regression and tutorial resource as the interface evolves.
...
\begin{itemize}
\item Dataset feature matrix for the VeloxChem documentation collection
\end{itemize}
