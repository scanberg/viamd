# VIAMD--VeloxChem Integration for Interactive Quantum Chemical Visualization and Analysis
...
The section should close by connecting implementation to the user-facing workflow: Summary queries metadata, SCF history, optimization data, and geometry from `md_vlx_t`; critical-point analysis follows an explicit density-to-topology pipeline (grid construction, state hashing, SCF density evaluation, extremum-graph extraction, optional simplification); Response uses excitation, oscillator/rotatory strength, transition-dipole, CPP, and vibrational arrays; Orbital Grid uses the same coefficient accessors and GTO volume evaluation as the main representation system.
